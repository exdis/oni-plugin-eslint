...
const activate = Oni => {
  const doLintForFile = async args => {
    ...
    console.log('Oni.editors.activeEditor.activeBuffer: ', Oni.editors.activeEditor.activeBuffer);
  ...
  async function executeEsLint(configPath, args, workingDirectory, autoFix) {
    ...
    processArgs = [...processArgs, '--format', 'json'];

    processArgs = [...processArgs, '--config', configPath];
  ...
  async function getLintConfig(workingDirectory) {
    ...
    //, { cwd: workingDirectory } used to restrict search
    try {
